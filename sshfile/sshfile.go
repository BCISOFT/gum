// Package sshfile provides an interface to pick a file from ssh.
...
type keymap sshfilepicker.KeyMap
...
func defaultKeymap() keymap {
	km := sshfilepicker.DefaultKeyMap()
	return keymap(km)
}
...
type model struct {
	...
	sshfilepicker sshfilepicker.Model
	selectedPath  string
	quitting      bool
	showHelp      bool
	padding       []int
...
func (m model) Init() tea.Cmd { return m.sshfilepicker.Init() }
...
func (m model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case tea.WindowSizeMsg:
		...
		m.sshfilepicker.SetHeight(height)
	case tea.KeyMsg:
		switch {
	...
	var cmd tea.Cmd
	m.sshfilepicker, cmd = m.sshfilepicker.Update(msg)
	if didSelect, path := m.sshfilepicker.DidSelectFile(msg); didSelect {
		m.selectedPath = path
		m.quitting = true
		return m, tea.Quit
	}
...
func (m model) View() string {
	...
	parts = append(parts, m.sshfilepicker.View())
	if m.showHelp {
